...
class Data:
    ...
    def __init__(self, args, dataset_type):
        set_seed(args.seed)
        max_seq_lengths = {'oos':30, 'stackoverflow':45, 'banking':55, 'snips':35, 'HWU64':25, 'ATIS':46}
        args.max_seq_length = max_seq_lengths[dataset_type]

        processor = DatasetProcessor()
        # self.data_dir = os.path.join(args.data_dir, args.dataset)
        self.data_dir = os.path.join(args.data_dir, dataset_type)
        ...
        self.step2_examples = self.get_examples(processor, args, 'train', args.few)
        ...
        self.step2_train_dataloader=self.get_loader(self.step2_examples, args, 'train')
    ...
    def get_examples(self, processor, args, mode='train', few=False):
        ori_examples = processor.get_examples(self.data_dir, mode)
        examples = []
        set_seed(args.seed)
        if mode == 'train':
            if few:
                for label in set([example.label for example in ori_examples]):
                    same_label_examples = [example for example in ori_examples if example.label == label]
                    sorted_examples = sorted(same_label_examples, key=lambda x: x.guid)
                    sampled_examples = sorted_examples[:args.shot]
